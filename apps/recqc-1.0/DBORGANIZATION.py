...
class DBOrganization(object):
    ...
    def write_sdf(self):
        output_sdf_path = "13c_generate11.sdf"
        ...
        for sdf_file in sdf_files:
            ...
            if os.path.exists(excel_file_path):
                ...
                for mol in supplier:
                    self.first_id += 1
                    mol.SetProp("ceshiID", str(self.first_id))
                    ...
                    for atoms in range(mol.GetNumAtoms()):
                        ...
                        if symbol == "C" or symbol == "c":
                            if int(atoms) + 1 in all_c_dict.keys():
                                ...
                                if num_hydrogens == 3:
                                    primaries_data = (
                                        f"{str(int(atoms) + 1)}\t{str(all_c_dict[int(atoms) + 1])}"
                                    )
                ...
                LOGGER.warning("No matching Excel file was found for the SDF input.")
        output_sdf.close()
