...
class ReceiveData:
    ...
    def back_match_type_state(self):
        if (
            self.match_state == 0
            or self.match_state == 4
            or self.match_state == 5
            or self.match_state == 6
        ):
...
class ProcesInputdata(ReceiveData):
    def __init__(
        self, all_spectrum_str, up_dept135_str, down_dept135_str, dept90_str, align_value=0.2
    ):
            ...
            for value135 in self.dept135:
                for index, allvalue in enumerate(self.all_spectrum):
                    ...
                    if abs(value135 - allvalue) <= self.align_value + 0.001:
                        ...
                        continue
            self.Quaternaries = self.list_difference(
                self.all_spectrum, remove_values2
            )  # 有问题，两个数字相同时
...
class ProductMatchData:
    def __init__(
        self, processinputdata_obj, process_sdf_obj
    ):  # 用户输入数据及类型对象；库文件处理结果对象
    ...
    def get_match_data(self):
        ...
        type_list = (
            ["Tertiaries"] * len(self.Tertiaries)
            + ["Secondaries"] * len(self.Secondaries)
            + ["Primaries"] * len(self.Primaries)
            + ["Quaternaries"] * len(self.Quaternaries)
            + ["psq"] * len(self.psq)
            + ["pt"] * len(self.pt)
            + ["pstq"] * len(self.pstq)
        )
