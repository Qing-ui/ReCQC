...
class SimilarSearch(object):
    def __init__(self, smiles, sdf_folder_path, threshold=0.8, radius=3):
        ...
        self.ref_fp = AllChem.GetMorganFingerprintAsBitVect(
            self.ref_mol, radius=self.radius, useFeatures=True
        )
                    ...
                    for mol in supplier:
                        if mol is not None:
                            mol_fp = AllChem.GetMorganFingerprintAsBitVect(
                                mol, radius=self.radius, useFeatures=True
                            )
        ...
        if similar_molecules:
            ...
            with Chem.SDWriter(output_sdf_path) as writer:
                for mol, similarity in similar_molecules:
                    writer.write(mol)
            print(
                f"Successfully written {len(similar_molecules)} similar molecules to {output_sdf_path}"
            )
        else:
        ...
        if molecules_with_substructure:
            ...
            with Chem.SDWriter(output_sdf_path) as writer:
                for mol in molecules_with_substructure:
                    writer.write(mol)
            print(
                f"Successfully written {len(molecules_with_substructure)} molecules with substructure to {output_sdf_path}"
            )
        else:
