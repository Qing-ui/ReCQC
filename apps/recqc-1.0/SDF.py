                ...
                for prop in properties_to_check:
                    try:
                        ...
                        if prop == "ID" and (prop_value is None or prop_value == ""):
                            # 对ID属性进行特殊处理，因为它不能为空
                            errors.append(
                                f"In the file {user_path}, the ID attribute of the {mol_index} molecule is absent or empty."
                            )
                        elif prop == "FW" and (prop_value is None or prop_value == ""):
                            # 对FW属性进行特殊处理，因为它不能为空
                            errors.append(
                                f"In the file {user_path}, the FW attribute of the {mol_index} molecule is absent or empty."
                            )

                    except KeyError:
                        # 如果属性不存在，则记录错误
                        errors.append(
                            f"In the file {user_path}, the {prop} attribute of the {mol_index} molecule does not exist. "
                        )
...
class Process_sdf_files:
    def __init__(
        self,
        file_paths_object,
        compound_shift_type_name,
        min_weight=0.0,
        max_weight=3000.0,
        len_min_c_atom_num=10,
        len_max_c_atom_num=100,
    ):
    ...
    def process_molecule(self, mol):
        ...
        if self.min_weight <= fw <= self.max_weight and carbon_count in range(
            self.min_c_atom_num, self.max_c_atom_num + 1
        ):
            compound_shift, compound_shift_type, compound_c_atom_num_list = self.extract_shift_data(
                mol, compound_id
            )
    ...
    def process_shift_string(
        self, shift_str, compound_c_atom_num_list, compound_shift, compound_shift_type, type_name
    ):
        error_list = []
            ...
            if line:  # 确保行不为空
                ...
                if len(parts) == 2:
                    try:
                        ...
                        error_list.append(
                            f"Error data row: {line}, data cannot be converted to integer or floating-point number"
                        )
                else:
                    error_list.append(
                        f"Error data row: {line}, which should contain two tab-separated fields"
                    )
        if error_list:
