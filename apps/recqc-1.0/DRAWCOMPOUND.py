...
class DrawCompound(object):
    ...
    def product_results(self):
        ...
        first_50_items = (
            list(islice(items_iterator, 20))
            if len(self.id_score_dict) > 50
            else list(self.id_score_dict.items())
        )
        ...
        for key, value in first_50_items:
            ...
            styled_df = result_dataframe.style.apply(
                self.row_color,
                axis=1,
            ...
            ppm_img2 = self.generate_ppm(
                green_user_data, yellow_user_data, orange_user_data, red_user_data
            )
            ...
            for k, v in mol_dict.items():
                if k == key:
                    ...
                    try:
                        index_data = self.get_data_type(
                            green_index, yellow_index, orange_index, red_index
                        )
                        db_data = self.get_data_type(
                            green_db_data, yellow_db_data, orange_db_data, red_db_data
                        )
                        user_data = self.get_data_type(
                            green_user_data, yellow_user_data, orange_user_data, red_user_data
                        )
                        compound_structure1 = self.draw_compound(
                            green_index,
                            yellow_index,
                            orange_index,
                            red_index,
                        ...
                        compound_structure2 = self.draw_compound(
                            green_index,
                            yellow_index,
                            orange_index,
                            red_index,
                        ...
                        compound_structure3 = self.draw_compound(
                            green_index,
                            yellow_index,
                            orange_index,
                            red_index,
                        ...
                        LOGGER.warning("Failed to render molecule %s: %s", key, exc)
        ...
        for file_path in filepaths:
            ...
            for mol in supplier:
                try:
                    ...
                    LOGGER.warning("Skipping an invalid molecule record: %s", exc)
        return mol_dict
