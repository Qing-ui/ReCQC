        ...
        style.theme_create(
            ...
            settings={
                ...
                "TButton": {
                    ...
                    "map": {"background": [("active", cls.COLORS["highlight"])]},
                },
                "TEntry": {
                    "configure": {
...
class BaseModeFrame(ttk.Frame):
    ...
    def create_widgets(self):
        ...
        ttk.Button(file_frame, text="Select the SDF file", command=self.select_files).pack(
            side=tk.LEFT, padx=5
        )
        ...
        ttk.Button(self.btn_frame, text="Start the analysis", command=self.start_analysis).pack(
            side=tk.LEFT, padx=5
        )
        ttk.Button(self.btn_frame, text="Reset parameter", command=self.reset_parameters).pack(
            side=tk.LEFT
        )
...
class CombinedModeFrame(BaseModeFrame):
    ...
    def create_carbon_params(self, parent):
        ...
        ttk.Radiobutton(
            mode_frame,
            text="C_typed",
            variable=self.c_mode,
            value="typed",
            command=self.toggle_c_params,
        ).pack(side=tk.LEFT)
        ttk.Radiobutton(
            mode_frame,
            text="C_untyped",
        ...
        ttk.Radiobutton(mode_frame, text="Y", variable=self.c_merge, value="Y").pack(side=tk.LEFT)
        ttk.Radiobutton(mode_frame, text="N", variable=self.c_merge, value="N").pack(side=tk.LEFT)
    ...
    def create_score_params(self, parent):
        ...
        ttk.Radiobutton(
            ...
            value="global",
            command=self.toggle_score_mode,
        ).pack(anchor=tk.W)
        ttk.Radiobutton(
            parent,
            text="Fine model",
        ...
        ttk.Button(
            btn_frame, text="+ Add range", command=lambda: self.add_range_row((0, 0), 0, 0)
        ).pack(pady=5)
    ...
    def add_range_row(self, carbon_range, green_val, yellow_val):
        ...
        # 删除按钮
        ttk.Button(frame, text="×", width=2, command=lambda: self.remove_range_row(frame)).pack(
    ...
    def create_hmqc_params(self, parent):
        ...
        ttk.Radiobutton(mode_frame, text="Y", variable=self.ch_merge, value="Y").pack(side=tk.LEFT)
        ttk.Radiobutton(mode_frame, text="N", variable=self.ch_merge, value="N").pack(side=tk.LEFT)
        mode_frame.pack(anchor=tk.W)
    ...
    def create_hmqc_input_groups(self):
        ...
        types_config = {1: ["All_type"], 2: ["type13", "type2"], 3: ["type1", "type2", "type3"]}
        ...
        try:
            ...
            for field in required_fields:
                ...
                if not re.match(pattern, text):
                    raise ValueError(
                        f"{field} format error, should be (numbers, numbers) list, support decimals, do not support negative numbers"
                    )
            return True
        ...
        if scoring_mode == "global":
            ...
            for row in self.range_rows:
                try:
                    ...
                    fine_ranges.append(((c_start, c_end), green_val, yellow_val))
                except ValueError:
                    continue
    ...
    def run_analysis(self):
        ...
        viewer = CombinedResultViewer(
            db_path="chem_data.db", results=results, output_dir="combined_results"
        )
...
class CarbonOnlyModeFrame(BaseModeFrame):
    ...
    def add_range_row(self, carbon_range, green_val, yellow_val):
        ...
        del_btn = ttk.Button(frame, text="×", width=2, command=lambda: self.remove_range_row(frame))
    ...
    def run_analysis(self):
        ...
        try:
            ...
            visualizer = CHMatchVisualizer(
                db_path="chem_data.db", top_results=results, mode=config["ch_mode"]
            )
            results = pipeline.execute()
...
class App(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("ReCQC 2.0 - Natural product analysis based on C spectrum and HSQC")
    ...
    def create_ui(self):
        ...
        ttk.Label(
            header,
            text="ReCQC 2.0",
            font=ChemTheme.FONTS["title"],
            foreground=ChemTheme.COLORS["highlight"],
        ).pack(pady=5)
        ttk.Label(
            header,
            text="Analysis of natural product mixtures based on C spectrum and HSQC spectrum",
            font=ChemTheme.FONTS["subtitle"],
        ).pack()
        header.pack(pady=20)
        ...
        for text, frame_class in modes:
            btn = ttk.Button(
                nav_frame, text=text, command=lambda fc=frame_class: self.show_mode(fc)
            )
