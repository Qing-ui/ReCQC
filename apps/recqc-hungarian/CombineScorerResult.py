...
class CombinedScorerGUI:
    ...
    def _validate_parameters(self):
        ...
        if self.c_range[0] > self.c_range[1]:
            raise ValueError(
                "The carbon number range is invalid, and the starting value needs to be less than the end value"
            )
        if self.fw_range[0] > self.fw_range[1]:
            raise ValueError(
                "The molecular weight range is invalid, and the start value needs to be less than the end value"
            )
        # 碳数据
    ...
    def _process_carbon_matching(self):
        ...
        molecule_c_scorer.process_molecules(
            self_weight=self.self_weight, env_weight=self.env_weight, env_level=self.env_level
        )
    ...
    def _process_ch_matching(self):
        ...
        ch_scorer = MoleculeScoreCalculator(
            db_path=self.db_path, c_tolerance=self.c_tol, h_tolerance=self.h_tol
        )
    ...
    def _get_combined_results(self, top_n: int = 20) -> List[Tuple]:
        ...
        # 执行联合评分
        CombinedScorer(
            db_path=self.db_path, c_weight=self.c_weight, ch_weight=self.ch_weight
        ).process()
    ...
    def _generate_c_score_plots(self, conn, mol, mol_id, name, output_dir):
        ...
        self._draw_labeled_molecule(
            ...
            highlight_colors=[self._color_to_rgb(c["color"]) for c in carbons],
            labels={
                c["index"]: f"{c['db_shift']:.1f}" for c in carbons if c.get("db_shift") is not None
            },
        ...
        self._draw_labeled_molecule(
            ...
            highlight_colors=[self._color_to_rgb(c["color"]) for c in carbons],
            labels={
                c["index"]: f"{c['user_shift']:.1f}"
                for c in carbons
                if c.get("user_shift") is not None
            },
    ...
    def _plot_carbon_visuals(self, mol: Chem.Mol, carbons: List[Dict], name: str, output_dir: str):
    ...
    def _draw_labeled_molecule(
        ...
        # 配置高亮样式
        highlight_dict = {
            atom_idx: color for atom_idx, color in zip(highlight_atoms, highlight_colors)
        }

        # 绘制分子
        drawer.DrawMolecule(mol, highlightAtoms=highlight_atoms, highlightAtomColors=highlight_dict)
...
class CombinedResultViewer:
    ...
    def create_result_window(self, parent):
        ...
        ttk.Button(control_frame, text="◀ Previous", command=lambda: self.show_page(-1)).pack(
            side=tk.LEFT, padx=5
        )
        ttk.Button(control_frame, text="Next ▶", command=lambda: self.show_page(1)).pack(
            side=tk.LEFT
        )
    ...
    def create_result_item(self, result, rank):
        ...
        ttk.Label(info_frame, text=f"Rank：{rank + 1}", font=("Arial", 10, "bold")).pack(
            anchor=tk.W
        )
        ttk.Label(
            info_frame, text=f"synthesis: {float(combined_score):.2f}", font=("Arial", 9)
        ).pack(anchor=tk.W)
        ttk.Label(
            info_frame, text=f"C: {c_score:.2f} | CH: {ch_score:.2f}", font=("Arial", 9)
        ).pack(anchor=tk.W)
    ...
    def show_detail(self, mol_id, mol_name):
        ...
        info_text = (
            f"ID: {mol_id} | name: {mol_name} | "
            f"Score: {score_data[0]:.2f} | "
            f"C_Score: {score_data[1]:.2f} | "
            f"CH_Score: {score_data[2]:.2f}"
        )
        tk.Label(header_frame, text=info_text, font=("Arial", 12), bg="#2C3E50", fg="white").pack(
            side=tk.LEFT
        )
        ...
        for text, img_type in [
            ...
            tk.Radiobutton(
                radio_frame,
                text=text,
                variable=self.carbon_img_type,
                value=img_type,
                command=lambda: self.update_carbon_img(mol_name),
                bg="#2C3E50",
                fg="white",
            ).pack(side=tk.LEFT)
        ...
        table_frame = tk.LabelFrame(
            detail_win, text="Carbon atom shift matching details", bg="#2C3E50", fg="white"
        )
