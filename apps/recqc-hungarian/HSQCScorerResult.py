...
class CHOnlyScorerGUI:
    ...
    def _save_run_params(self):
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            conn.execute(
                "CREATE TABLE IF NOT EXISTS hsqc_run_params (id INTEGER PRIMARY KEY CHECK (id = 1), c_tol REAL, h_tol REAL)"
            )
            conn.execute(
                "INSERT OR REPLACE INTO hsqc_run_params (id, c_tol, h_tol) VALUES (1, ?, ?)",
                (self.c_tol, self.h_tol),
            )
            conn.commit()
...
class CHMatchVisualizer:
    ...
    def __init__(self, db_path: str, top_results: List[Tuple], mode: int = 2):
        """
        :param db_path: 数据库路径
    ...
    def _plot_single(self, points: List[Tuple], title: str, filename: str):
        ...
        # Expand axes to include all observed coordinates.
        if points:
...
class CHResultViewer:
    ...
    def _load_tolerances(self):
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                row = conn.execute(
                    "SELECT c_tol, h_tol FROM hsqc_run_params WHERE id = 1"
                ).fetchone()
                if row:
    ...
    def _count_in_tolerance(self, mol_id):
        ...
        with sqlite3.connect(self.db_path) as conn:
            conn.row_factory = sqlite3.Row
            cursor = conn.execute(
                "SELECT db_point, user_point FROM user_matches WHERE mol_id = ? AND distance < 1e9",
                (mol_id,),
            )
            for row in cursor:
    ...
    def create_result_window(self, parent):
        ...
        ttk.Button(control_frame, text="◀ Previous page", command=lambda: self.show_page(-1)).pack(
            side=tk.LEFT, padx=5
        )
        ttk.Button(control_frame, text="Next page ▶", command=lambda: self.show_page(1)).pack(
            side=tk.LEFT
        )
    ...
    def create_result_item(self, result, rank):
        ...
        # 详情按钮
        ttk.Button(
            info_frame, text="View details", command=lambda mid=mol_id: self.show_detail(mid)
        ).pack(pady=2)
    ...
    def show_detail(self, mol_id):
        ...
        in_tol = self._count_in_tolerance(mol_id)
        in_tol_text = (
            f" | In-tolerance points: {in_tol}/{score_data[2]}" if in_tol is not None else ""
        )
        tk.Label(
            header_frame,
        ...
        # 左侧结构图
        struct_frame = tk.LabelFrame(
            viz_frame, text="Molecular structure", bg="#2C3E50", fg="white"
        )
        ...
        ttk.Radiobutton(
            ...
            variable=self.coord_img_type,
            value="lib",
            command=lambda: self.update_coord_img(mol_id),
        ).pack(side=tk.LEFT)
        ttk.Radiobutton(
            coord_frame,
            text="User matching",
        ...
        table_frame = tk.LabelFrame(
            main_frame, text="Matching point details", bg="#2C3E50", fg="white", height=350
        )
        ...
        columns = (
            "Type",
            "C Displacement (Library)",
            "H Displacement (Library)",
            "C Displacement (User)",
            "H Displacement (User)",
            "Distance",
            "Point score",
    ...
    def get_type_name(self, h_type):
        """获取H类型名称"""
        return {1: "CH", 2: "CH2", 3: "CH3"}.get(h_type, "Unknown")
