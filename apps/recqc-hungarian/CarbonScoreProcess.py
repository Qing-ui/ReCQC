...
class GreedyMatcher:
    ...
    def _validate_inputs(self):
        """参数校验（支持两种模式）"""
        if self.mode == "typed":
            if not isinstance(self.user_data, dict) or any(
                not isinstance(v, list) for v in self.user_data.values()
            ):
        ...
        with self._get_conn() as conn:
            ...
            for row in rows:
                ...
                if mol_id not in self.carbon_cache:
                    self.carbon_cache[mol_id] = {"name": name, "carbons": {}}
    ...
    def _vectorized_match(self, user_shifts, db_candidates):
        ...
        try:
            ...
            if self.C_merge == "N":
                ...
                row_greens = [
                    self._get_green_threshold_for_shift(carbon["c_shift"]) for carbon, _ in db_items
                ]
                ...
                for row_idx, col_idx in assigned.items():
                    ...
                    matches.append((original_user_shift, carbon["c_index"], carbon["c_shift"]))
                    matched_c_idx.add(carbon["c_index"])

                unmatched = [
                    carbon for carbon, _ in db_items if carbon["c_index"] not in matched_c_idx
                ]
            ...
            if self.C_merge == "Y":
                ...
                row_greens = [
                    self._get_green_threshold_for_shift(group_shift)
                    for _, group_shift, _ in group_items
                ]
                col_values = [user_shift_float for _, _, user_shift_float in user_items]
        ...
        try:
            ...
            if self.mode == "typed":
                ...
                remaining_candidates = [
                    c for c in mol_data["carbons"].values() if c["c_index"] not in matched_c_idx
                ]
                ...
                if remaining_candidates and remaining_user:
                    matches, unmatched = self._vectorized_match(
                        remaining_user, remaining_candidates
                    )
                    for u_shift, c_idx, c_shift in matches:
                    ...
                    for carbon in unmatched:
                        all_matches.append(
                            (None, None, carbon["c_index"], carbon.get("c_shift"), False)
                        )
                else:
                    # 第二阶段无法匹配时，剩余碳全部写占位
        ...
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            with tqdm(total=len(qualified), desc="Match progress") as pbar:
                for batch in batches:
                    futures = {
                        executor.submit(self._process_molecule, mol_id): mol_id for mol_id in batch
                    }
...
class CarbonScorer:
    ...
    def _global_score(
        self, delta: float, type_matched: bool, thresholds: Tuple[float, float]
    ) -> Tuple[float, str]:
        """全局模式评分"""
...
class RobustMoleculeScorer:
    ...
    def process_molecules(self, self_weight: float, env_weight: float, env_level: int = 1):
        """
        增强鲁棒性的评分流程
        ...
        with sqlite3.connect(self.db_path) as conn:
            conn.row_factory = sqlite3.Row
            mol_ids = [row[0] for row in conn.execute("SELECT DISTINCT mol_id FROM greedy_matches")]
    ...
    def _load_molecule_data(self, conn, mol_id: int) -> Dict[int, dict]:
        ...
        for row in cursor:
            ...
            try:
                ...
                raise ValueError(
                    f"Carbon atom {row['c_index']} of molecule {mol_id} is malformed"
                ) from e

            molecule_data[row["c_index"]] = {"score": row["score"], "connections": connections}
    ...
    def _get_environment_atoms(
        self, root: int, graph: Dict[int, List[int]], max_level: int
    ) -> List[int]:
        """获取多级环境原子（带连接有效性检查）"""
        visited = {root}
        queue = deque([(root, 0)])
    ...
    def _calculate_env_score(self, env_atoms: List[int], mol_data: dict) -> float:
        """计算环境评分（带数据完整性检查）"""
        if len(env_atoms) == 0:
            return 1
        else:
            ...
            for atom in env_atoms:
                if atom not in mol_data:
                    logging.warning(
                        f" Ignore environmental atoms that do not exist in the molecular data:{atom}"
                    )
                    continue
    ...
    def _score_molecule(
        ...
        for c_index, data in mol_data.items():
            ...
            if data["score"] == 0:
                ...
                composite = self_weight * data["score"] + env_weight * env_score
        ...
        conn.execute(
            ...
            VALUES (?, ?, ?, ?)
        """,
            (mol_id, mol_name, round(final_score, 4), json.dumps(score_details)),
        )
        conn.commit()
