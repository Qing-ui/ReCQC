...
class CarbonResultVisualizer:
    ...
    def _plot_structure(
        self, mol: Chem.Mol, carbons: List[Dict], mol_id: int, name: str, output_dir: str
    ):
    ...
    def _plot_shift_intensity(self, carbons: List[Dict], mol_id: int, name: str, output_dir: str):
...
class ResultViewer:
    ...
    def create_result_item(self, result, rank):
        ...
        ttk.Label(info_frame, text=f"synthesis: {float(env_score):.2f}", font=("Arial", 9)).pack(
            anchor=tk.W
        )
    ...
    def show_detail(self, mol_id):
        ...
        for text, img_type in [
            ...
            tk.Radiobutton(
                radio_frame,
                text=text,
                variable=self.struct_img_type,
                value=img_type,
                command=lambda: self.update_struct_img(),
                bg="#2C3E50",
                fg="white",
                selectcolor="#34495E",
            ).pack(side=tk.LEFT)
        ...
        table_frame = tk.LabelFrame(
            main_frame, text="Carbon Atom Shift Matching List", bg="#2C3E50", fg="white", height=350
        )
